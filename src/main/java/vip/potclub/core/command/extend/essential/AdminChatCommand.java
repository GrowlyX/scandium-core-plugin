...
public class AdminChatCommand extends BaseCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (player.hasPermission(ChatChannel.ADMIN.getPermission())) {
            if (args.length == 0) {
            ...
            if (args.length > 0) {
                ...
                CorePlugin.getInstance().getRedisThread().execute(() -> client.write(RedisUtil.onChatChannel(ChatChannel.ADMIN, message, player)));
            }
        } else {
